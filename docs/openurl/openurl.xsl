...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="4">
            <xsl:for-each select="oai:header/oai:setSpec[generate-id(.)=generate-id(key('setSpecs',.)[1])]">
                ...
                <xsl:if test="starts-with(.,$temp)">
                    ...
                    <tr><td><strong><a><xsl:attribute name="href"><xsl:text>/<xsl:value-of select="."/>/ListRecords.oai_dc.xml</xsl:text></xsl:attribute>
    ...
    <xsl:template match="oai:setName">
        ...
        <td><strong><a><xsl:attribute name="href">/<xsl:value-of select="../oai:setSpec"/>/ListRecords.oai_dc.xml</xsl:attribute>
    ...
    <xsl:template match="oai:metadataFormat">
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <tr valign="top"><td><a><xsl:attribute name="href">ListRecords.<xsl:value-of select="oai:metadataPrefix"/>.xml</xsl:attribute><xsl:choose><xsl:when test="oai:metadataPrefix='oai_dc'"></xsl:when><xsl:when test="oai:metadataPrefix='mtx'">Z39.88-2004 MTX definition</xsl:when><xsl:when test="oai:metadataPrefix='pro'">Community Profile Format</xsl:when><xsl:when test="oai:metadataPrefix='xsd'">XML Schema definition</xsl:when><xsl:otherwise><xsl:value-of select="oai:metadataPrefix"/></xsl:otherwise></xsl:choose></a></td></tr>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="dc:type" mode="verbose">
        <tr valign="top">
            ...
            <td><a><xsl:attribute name="href"><xsl:value-of select="."/>/<xsl:text>ListRecords.oai_dc.xml</xsl:text></xsl:attribute><xsl:value-of select="."/></a></td>
        </tr>
    </xsl:template>
